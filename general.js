Vue.component('logoElem',{
	props: ['imgSrc','imgAlt','modalCopy','modalLink'],
	template: `<img
		v-bind:src="'imgs/logo/'+imgSrc+'.gif'"
		v-bind:modal-copy="modalCopy"
		v-bind:modal-link="modalLink"
		v-bind:alt="imgAlt"
		v-on:click="callModal(this)"
	></img>`,
	methods: {
	    callModal: function(){
			var colors=['pink','blue','yellow'];
			var randColor=colors[(Math.random()*colors.length) << 0];
			...
			document.getElementById('modal').innerHTML+=`<div class="modalback" onClick="this.outerHTML='';document.body.classList='';">
			 	<div class="modal `+randColor+`">
			 		<h5>`+this.imgAlt+`<img src="imgs/cross.svg" alt="close button" /></h5>
			 		<img src="imgs/logo/`+this.imgSrc+`.gif" alt="`+this.imgAlt+`" />
			 		<p>`+this.modalCopy+`</p>
			 		<a href="`+this.modalLink+`" target="_blank">`+this.modalLink+`</a>
			 	</div>
			</div>`;
		}
	}
...
var App = new Vue({
	...
	template: `<div class="logos"><logoElem
		v-for="logo in logos"
		v-bind:key="logo.imgAlt"
		v-bind:imgSrc="logo.imgSrc"
		v-bind:modalCopy="logo.modalCopy"
		v-bind:modalLink="logo.modalLink"
		v-bind:imgAlt="logo.imgAlt"
		></logoElem></div>`
})